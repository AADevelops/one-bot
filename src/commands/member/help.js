// TODO: Make sections based on categories.

module.exports = {
    name: "help",
    description: "Displays all commands.",
    usage: `${process.env.PREFIX}help <category or commandName>`,
    execute(message, args) {
        const { commands } = message.client
        const information = [];

        if (!args.length) {
            message.channel.send(`Commands: ${commands.map(command => command.name).join(", ")} ${"\n"}If you would like to have more detail for a command, use \`;help <commandName>\``);
        } else if (args.length) {
